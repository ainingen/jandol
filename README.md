...
- **立ち絵とセリフ** … いま打っている人の顔と一言が出る。縦持ちは自分の捨て牌の下に
  顔と吹き出しの帯、横持ちの広い画面（900px以上）では右の余白に立ち絵。
  セリフは性格19種 × 8場面で323本。リーチ・ロン・ツモ・放銃・鳴き・流局のほか、
  手番がまわると1割ほどの確率で雑談する
...
| 立ち絵の大きさ・出る条件 | `src/match.css` の `#tachie`（`min-width:900px`） |
| セリフ | `src/serifu.js` の `LINES`（性格名が鍵） |
| 雑談が出る確率 | `src/ui.js` の `renderTachie` の `0.12` |
