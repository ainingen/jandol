...
# **thumb/ を忘れないこと**——大会のカードと対局画面の顔がここを読むので、
# 無いと出走表も中継も決勝卓も、席プレートも順位表も影絵になる（画面は普通に動く）
...
tools/make-thumbs.py 顔を 176×234 へ焼き直して thumb/ に出す（大会のカードと対局画面）
...
thumb/              その 176×234 版（212枚・1.0MB）。**大会のカードと対局画面が読む**
                    （席プレート・カットイン・順位表。立ち絵は img/ のまま）。
...
| 公開の版（表紙に出る番号） | `src/title.js` の `VERSION`（**上げるのはここだけ。**`resume.js` の `V` とは別物） |
| 版に添える断り（暫定版・意見の宛先） | `src/title.js` の `renderTop` の `.ttVer`（**文言を変えたら `make-font.py`**） |
...
### 版と断りは、逆に**DOMへ置く**（2026年9月10日）

表紙の釦の下に出している二行——`v0.5.0　暫定版` と
`ご意見・不具合は PLiCy のコメントへお願いします`——は **DOM側**（`.ttVer`）。
**canvasに描かないこと。**

- **canvasに描くと、版がサムネイルに焼き付く。**上のとおりPLiCyは
  最初のcanvasの中身を撮るので、**版を上げるたびに看板の絵が変わる。**
  「暫定版」の文字が看板に出続けるのも避けたい
- **DOMなら画面には見えて、サムネイルには写らない。**
  制作タグとちょうど逆の判断で、**理由は同じ一つの事実**
  ——「撮られるのはcanvasだけ」。**看板に残したいものはcanvas、
  版のように移り変わるものはDOM**、と分けて考えること
- 確かめかたは **canvasだけを `toDataURL` で吸い出して見る**のがいちばん確か。
  画面のスクリーンショットではDOMも一緒に写るので判別できない

**書体は 700 のまま**（`font-weight` を 800 にしないこと）。理由は制作タグと同じ。

**版を上げる手順は「`src/title.js` の `VERSION` を書き換えて `python3 build.py`」だけ。**
他に触る場所は無い（表紙が読んで出すだけで、判定にも保存にも使っていない）。
**番号ではなく文言のほうを変えたとき**（「暫定版」を別の語にする、宛先を変える）だけ、
`python3 tools/make-font.py` も回すこと——数字と英字はもともと入っている。

**この二行を足したぶん、`#cover` の `max-height` の引き算も増やしてある**
（`src/title.css`。410→456 / 355→401）。表紙の高さは
「画面の高さ − 表紙以外の中身」で決めているので、**中身を足したら必ずここも足す。**
...
- **最後の一行は副題（`Title.SUBTITLE`）と対。**片方だけ動かすと
  `tools/test-prologue.js` が落ちる（比べるのは末尾の句点を落とした形
  ——本文は地の文なので句点で終わり、副題は看板なので持たない）。
...
- **書体は `var(--mincho)`。丸ゴ（`Maru`）を使わないこと。**
  ...
  **本文のためにフォントを焼き直さないこと**——450字ぶんの漢字を足すと
  woff2 が太り、表紙の描画が遅くなる（canvas は `document.fonts.load` を待っている）
