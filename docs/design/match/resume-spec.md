...
**`g.run()` が返った直後**（`match.js:445` の次）。半荘が正常に終わった＝復帰するものが無い。

`showResult`（順位の表示）の前に消す。**結果を見ている最中に落ちたら結果は見られないが、それ以上は失わない。**ここで消さずに `showResult` の後にすると、結果の表示中に落ちた場合、最後の局を打ち直すことになる。そのほうが悪い。

`UI.giveUp`（おまかせ／早送り）は対局を最後まで進めるので、通常の終了と同じ経路で消える。特別扱いは要らない。
...
- **結果の表示中に落ちた場合**（§4）。いまは「結果が見られない」で済ませる
