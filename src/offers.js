/* ============================================================
   ...
   **`parlor.day` を見ない。**日付で発火させると、あとから足した依頼が
   ...
   ------------------------------------------------------------
   アイドル案件を足すとき — 一行に何を書くか（§8.2）
   ------------------------------------------------------------
   下の `IDOL_OFFERS` に一行足せば、それだけで届いて・受けられて・
   夜の結果カードまで出る。**足すのは表の一行だけ**（§1.3 の運営型）。
   書くのはこれだけ:
   ...
   **`res` は四通り。**向いていたか（`fit` に `chara` があるか）×
   勝ったか（`places[c.id] === 1`）で、`Office.idolTell` が引く。

     res: {
       hitWin:  { head: '…', line: '…' },   向いていて勝った（match のみ）
       hit:     { head: '…', line: '…' },   向いていた
       missWin: { head: '…', line: '…' },   向いていないが勝った（match のみ）
       miss:    { head: '…', line: '…' },   向いていなかった
     }

   - **`match` が偽なら `hit` / `miss` の二つだけ書く。**
     「勝った側」を書くと、存在しない勝負を書くことになる
   - **`head` は見出し、`line` はその子の言葉**（鉤括弧は表示側が付ける）
   ...
   **案件ごとの固有名詞**（巻頭・会場・ハガキ・黒板・同時視聴・大将戦・
   スタジオ・平積み）を見出しに入れて分ける。
   `tools/test-office.js` が、案件をまたいで共有している語がゼロであることを
   機械的に見ている（同じ案件の中で繰り返すのは可）。

   **`fit` の配りを数え直すこと。**足す前は4件しか無く、
   **人見知り・生意気な後輩・毒舌・中二病気質の4種に出番が無かった**
   ——その性格の子を契約しても、アイドル案件でいちども「向いていた」が出ない。
   いまは10件で19種が2件ずつ（明るい元気娘とおっとりだけ3件）。
   **案件を足したら、19種の数を数え直して偏りを均すこと。**
   `tools/test-office.js` が「出番ゼロの性格が無いこと」と
   「いちばん多い性格との差が1件まで」を錠にしている。

   **大きい案件には条件（`when`）を付けること。**条件が無いと序盤から来て、
   当たりくじがそれ一つに偏る。**しかも、大きい案件どうしで軸を分けること**
   ——同じ条件にすると二つ同時に解禁されて、また偏る。いまは

     テレビ対局  事務所の格の軸  所属3人 ＋ ランク2 ＋ 大会に出たことがある
     写真集      人の軸          誰かの pop 78 ＋ その子の favor 60

   `when` が読む `roster` の `pop` / `favor` は `rosterOf` が作った値。
   **`popOf` の式をここに書き写さないこと**（§8.2）。
   ============================================================ */
