...
def source_chars():
    ...
    **src/*.js を丸ごと拾うので、地の文もここに入る。**
    いまは `src/prologue.js` の `PROLOGUE`（プロローグの本文）がそれで、
    **あれは明朝（`var(--mincho)`）で出しているので丸ゴには要らない。**
    明朝で出す画面の文章をここへ足していくと、使われない字で
    woff2 が太る——表紙の描画は `document.fonts.load` の完了を待っているので、
    太らせたぶんだけ表紙が遅れる。

    **実測（2026年9月11日・プロローグを入れた前後）：**

        fonts/maru-ui.woff2   584,384 バイト → 584,384 バイト（差分 0）
        拾った字              3,514字 → 3,514字（差分 0）

    `src/prologue.js` を退けて焼いたものと、置いて焼いたものが
    **1バイトも違わなかった。**本文の異なり字 161字の内訳は

        かな・記号  69字  … KANA_RANGES が丸ごと入れている
        常用漢字    90字  … joyo_chars() が入れている
        それ以外     2字  … ─ と 牌。どちらも他のソースに既に出ている

    で、**新しく増える字がゼロだったため。**
    「ほぼ太らないはず」ではなく、実際に0。
    ただしこれは**いまの本文がたまたま常用漢字で書けている**という話なので、
    **文章を足したらもう一度この二つ（バイト数と字数）を測って、
    ここを書き直すこと。**
    """
    import glob
    text = ''
