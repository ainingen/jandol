...
/* **比べるのは句点を落とした形。**本文は地の文なので句点で終わるが
   （spec §2 の確定稿がそう書いてある）、副題は看板なので句点を持たない。
   **違ってよいのは末尾の句点ひとつだけ**——それ以外が動いたら、
   片方だけ直したということなのでここが落ちる。
   本文を直すときは `Title.SUBTITLE` も一緒に動かすこと（spec §2）。 */
const dropKuten = (s) => s.replace(/。$/, '');
eq(dropKuten(lastLine), Title.SUBTITLE, '最後の一行は副題と対');
ok(lastLine === Title.SUBTITLE || lastLine === Title.SUBTITLE + '。',
  '副題との違いは末尾の句点だけ', J(lastLine));
